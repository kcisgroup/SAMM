...
def test_epoch(model, data_loader, e):
    ...
    acc, f1, pre, rec = eva(all_labels, predicted_labels)
    return acc, f1, pre, rec, all_labels, all_clusters
...
def solver(args, model, train_loader):
    ...
    for e in range(args.epoch):
        ...
        acc, f1, pre, rec, all_labels, all_clusters = test_epoch(model, train_loader, e)
        if best_acc < acc:
            best_acc = acc
            best_epoch = e+1
        ...
        print('\nEpoch: {:02d} | ACC: {:.4f} | f1: {:.4f} | PRE:{:.4f} | REC : {:.4f}\n'.format(e + 1, acc, f1, pre, rec))
    ...
    acc, f1, pre, rec = eva(best_labels, predicted_labels)


    print('\nACC: {:.4f} | f1: {:.4f} | PRE:{:.4f} | REC : {:.4f}\n'.format(acc, f1, pre, rec))

    return  acc_list, f1_list, pre_list, rec_list
...
if __name__ == '__main__':
    ...
    # Dataset parameters
    parser.add_argument('--train_path', default='./datas/breast_cancer/test.csv', help='dataset path')
    parser.add_argument('--feature_list', default='./datas/breast_cancer/list.txt', help='feature list path')
    parser.add_argument('--feature_dim', default=43, type=int, help='the feature num of data point')
    ...
    parser.add_argument('--lr', default=1e-4, type=float, help='the learning rate of the model')
